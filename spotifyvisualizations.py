...
cur.execute("SELECT * FROM spotifyGenres")
genre_list = cur.fetchall()

cur.execute("SELECT * FROM spotifyTracks")
track_list = cur.fetchall()

# list of (track, genre) tuples
genres_and_name_length_list = []
for i in range(len(genre_list)):
    genres_and_name_length_list.append((track_list[i][1], genre_list[i][1]))
